import { Command, CheckCircle2, Zap } from 'lucide-react';

interface BottomBarProps {
  onOpenCmd: () => void;
}

export default function BottomBar({ onOpenCmd }: BottomBarProps) {
  return (
    <footer className="h-10 border-t border-nexus-border flex items-center justify-between px-4 shrink-0 bg-nexus-bg text-xs font-mono select-none">
      <div className="flex items-center gap-4 text-nexus-dim">
        ...
        <div className="flex items-center gap-2 hidden md:flex">
          ...
          <span>1/3 Daily Priorities</span>
        </div>
      </div>

      <div className="flex items-center gap-4 text-nexus-dim">
        <div className="flex items-center gap-1 hidden sm:flex">
          <span className="w-1.5 h-1.5 rounded-full bg-nexus-mint" />
